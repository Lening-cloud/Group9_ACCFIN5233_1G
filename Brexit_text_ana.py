...
for i in range(len(list)):
    ...
    with open(path, 'r', encoding='utf-8') as brexit_read:
        brexit_string += brexit_read.read()
        brexit_string += '\n'
...
for k in brexit_split:
    ...
    for i in range(len(tokens_temp)):
        tokens_temp[i] = lemmatizer.lemmatize(tokens_temp[i], pos = get_wordnet_pos(tagged_sent[i][1]) or wordnet.NOUN)


    def get_wordnet_pos(tag):
        if tag.startswith('J'):
            return wordnet.ADJ
        # (omit) else if other word types...


    for i in range(len(tokens_temp)):
        tokens_temp[i] = lemmatizer.lemmatize(tokens_temp[i],
                                              pos=get_wordnet_pos(tagged_sent[i][1])
                                                  or wordnet.NOUN)
...
plt.savefig("result_{}/wordcloud_{}.png".format(year,year), format='png', dpi=500)
plt.show()
############## END NLP ####################
...
with open('result_{}/Posi_{}.txt'.format(year,year), 'w+') as f:
...
with open('result_{}/Nega_{}.txt'.format(year,year), 'w+') as f:
